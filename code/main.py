...
class Statistics_ai:
    ...
    def mean_hypothesis_test(self, noll_df, other_df):
        ...
        print(
            f"Hypothesis rejected because t_statistic({result.statistic}) is greater than {norm.ppf(0.95)} which is the critical value"
            if result.statistic > norm.ppf(0.95)
            else f"hypothesis not rejected since t_statistic({result.statistic}) is less than {norm.ppf(0.95)} which is the critical value"
        )
    ...
    def prediction_interval_out_of_our_sample_space(self, df, sub_df):
        ...
        ypred_testpoints = model.predict(points)
    ...
    def prediction_interval_in_our_sample_space(self, df, sub_df):
        df = self._df_list[df]
        model = ols(f"{sub_df} ~ {self._test}", data=df).fit()
        predictions = model.get_prediction(df[self._test])
        ...
        order = np.argsort(df[self._test])
        xs = np.array(df[self._test])[order]
        # ys = np.array(frame['obs_ci_lower'])[order]

        fig, ax = plt.subplots(figsize=(10, 5))

        sns.scatterplot(df, x=self._test, y=sub_df)
        ax.plot(df[self._test], ypred, "r", label="OLS prediction")

        order = np.argsort(df[self._test])
        x = np.array(df[self._test])[order]
        lower = np.array(frame["obs_ci_lower"])[order]
        upper = np.array(frame["obs_ci_upper"])[order]
        
        # prediction for the random points
        ypred_testpoints = model.predict(points)
        
        # plotting of the random points
        sns.scatterplot(
            x=points[self._test],
            y=ypred_testpoints,
            zorder=10,
            color="black",
            marker="s",
            label="OLS Predictions for test points",
        )
